...
class Compiler {
  ...
  // run方法启动编译，同时run方法接收外部传递的callback
  run(callback) {
    ...
    // 导出列表之后将每个chunks转化为单独的文件加入到输出列表assets中
  ...
  buildEntryModule(entry) {
    Object.keys(entry).forEach(entryName => {
      ...
      // 根据当前入口文件数量和模块的相互依赖关系，组成成为一个个包含当前入口所有依赖的chunk
  ...
  handleWebpackCompiler(moduleName, modulePath) {
    ...
    const module = {
      id: moduleId,
      dependencies: new Set(), // 改模块所依模块绝对路径地址
      name: [moduleName], // 改模块所属的入口文件
    }
    // 调用babel分析我们的代码
    ...
    traverse(ast, {
      ...
      CallExpression: nodePath => {
        ...
        if (node.callee.name === 'require') {
          ...
          // 通过babel修改源代码中的 require 变成 __webpack_require__ 语句
          node.callee = t.identifier('__webpack_require__')
          // 修改源代码中的require语句引入的模块 全部修改变为相对根路径来处理
    ...
    module.dependencies.forEach(dependency => {
      ...
      if (!path.isAbsolute(dependency)) {
        dependency = toUnixPath(path.join(this.rootPath, dependency))
      }
